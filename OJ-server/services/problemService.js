...
const editProblem = function(newProblem) {
    return new Promise((resolve, reject) => {
        ProblemModel.findOne({name: newProblem.name}, (err, mongoProblem) => {
            if (!mongoProblem) {
                reject('Problem doesn\'t exist');
            } else {
                mongoProblem.desc = newProblem.desc;
                mongoProblem.difficulty = newProblem.difficulty;
                mongoProblem.save();
                resolve(mongoProblem);
            }
        });
